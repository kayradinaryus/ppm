// features/open-houses.js - GÜNCELLENMİŞ
...
function injectOpenHouseHTML() {
    // Hata ayıklama için konsol logları eklendi
    const logPrefix = '[Açık Evler]:';
    ...
    if (cityNode.singleNodeValue) {
        ...
        console.log(logPrefix, `Bulunan şehir ID: ${cityID}`);

        if (openHousesDB.hasOwnProperty(cityID)) {
            console.log(logPrefix, `${cityID} ID'li şehir için veritabanında evler bulundu.`);
            
            // DÜZELTME: Menüyü eklemek için yeni ve daha sağlam bir hedef seçici kullanıyoruz.
            // Artık 'Seyahat' başlığını arayacağız.
            const TRAVEL_MENU_HEADER_XPATH = '//h3[contains(text(), "Seyahat")]';
            let travelHeaderHelper = new XPathHelper(TRAVEL_MENU_HEADER_XPATH);
            let travelHeaderNode = travelHeaderHelper.getFirstOrderedNode(document);

            if (travelHeaderNode.singleNodeValue) {
                let travelMenuDiv = travelHeaderNode.singleNodeValue.parentNode; // h3'ün parent'ı olan <div class="menu">
                console.log(logPrefix, "'Seyahat' menüsü bulundu, 'Açık Evler' menüsü ekleniyor...");
                ...
                newMenuH3.textContent = 'Açık Evler';
                ...
                openHousesDB[cityID]
                    ...
                    .forEach((houseDetails) => {
                        if (houseDetails.hasOwnProperty('name') && houseDetails.hasOwnProperty('id')) {
                            ...
                            newMenuUL.appendChild(newMenuLI); // li'yi ul'ye ekle
                            foundHouses++;
                        }
                    });

                if (foundHouses > 0) {
                    // DÜZELTME: Yeni menümüzü, 'Seyahat' menüsünün hemen üstüne ekliyoruz.
                    travelMenuDiv.parentNode.insertBefore(newMenuDiv, travelMenuDiv);
                    console.log(logPrefix, `${foundHouses} adet açık ev başarıyla eklendi.`);
                }
            } else {
                console.error(logPrefix, "Menüyü ekleyecek yer ('Seyahat' başlığı) bulunamadı.", true);
            }
        } else {
            console.log(logPrefix, `Bu şehir (${cityID}) için veritabanında açık ev kaydı yok.`);
        }
    } else {
        console.error(logPrefix, "Mevcut şehir ID'si bulunamadı.", true);
    }
}
